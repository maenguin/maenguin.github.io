...
### 구명보트 [`commit c88fd`](https://github.com/maenguin/Algorithm/commit/c88fd7aba387d5525efffe7bff9fc0629f1cc9e7)
사람의 체중 배열이 주어졌을때 두명만 탈 수 있는 제한된 무게의 보트가 몇개 필요한지 구하는 문제였다.  
처음에는 2중 for문으로 사람을 두명씩 전부 비교해 무게 합이 적절한 경우를 일일히 찾았다.  
이 알고리즘은 문제는 맞았지만 효율성 문제를 통과하지는 못했다.  
생각을 바꿔서 미리 sorting을 하고 while문 하나에서 양쪽 값을 가져오는 형식으로 처리했다.  
결과는 성공이었고 코드까지 직관적이고 깔끔해졌다. 정말 알고리즘은 어떻게 접근하느냐에 따라 어렵게 지저분하게 풀 수 도 있고 쉽고 깔끔하게 풀 수 도 있는듯 하다.  
***
...
### 정렬
#### 역순 정렬 하기
내장 compareTo 함수를 사용해 역정렬을 하고 싶을때는 아래 두가지 중 하나를 사용하면 된다.
```java
