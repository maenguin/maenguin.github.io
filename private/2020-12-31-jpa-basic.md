...
* **필드와 컬럼 매핑** : @Column, @Enumerated, @Lob, @Transient, @Temporal
* **기본 키 매핑** : @Id
* **연관관계 매핑** : @ManyToOne, @OneToOne, @ManyToMany, @JoinColum  
...
* **name**  
  ...
  카멜케이스로 적으면 자동으로 `_`로 변환되어 매핑된다.(ex orderHistory -> order_history)  
* **catalog**  
  DB의 catalog를 매핑한다.  
* **schema**  
  DB의 schema를 매핑한다.
...
여기서 JPA는 ID값이 있어서 영속할 수 있기 때문에 IDENTITY 전략은 다른 전략과는 예외적으로  
...
IDENTITY 전략과는 다르게 em.persist()시 시퀀스 값만 call하고 INSERT SQL을 실행하지 않기 때문에 버퍼링을 할 수 있지만  
성능 이슈가 문제될 수 있다.  
하지만 allocationSize를 설정해서 성능 최적화를 할 수 있다.  
